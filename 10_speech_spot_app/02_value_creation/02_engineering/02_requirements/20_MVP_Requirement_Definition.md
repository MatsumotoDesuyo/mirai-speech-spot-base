...
## 1. Introduction
* **Project:** みらい街頭演説Base
* **Version:** 1.5 (Pin Position Adjustment)
...
* **Position:** 「位置を調整」ボタンから位置調整モードに遷移し、ピンの緯度・経度を変更可能（詳細は3.8参照）。
...
### 3.8. Pin Position Adjustment (ピン位置調整)
* **Trigger:** 投稿フォーム（新規・編集共通）の位置情報エリアにある「位置を調整」ボタン。
* **UI Flow:**
...
* **Animation:** 位置調整モードの開始時・確定時・キャンセル時にflyToアニメーションで滑らかに移動。
* **Interaction Restrictions:** 位置調整モード中は長押し/右クリックによる新規登録、ピンタップ、ナビゲーションコントロール、現在地ボタンが無効化される。

### 3.9. Admin Deletion (管理機能)
