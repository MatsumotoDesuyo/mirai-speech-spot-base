...
## 1. Introduction
* **Project:** みらい街頭演説Base
* **Version:** 1.4 (Image Optional - Trial Operation)
...
* **Interaction:**
    ...
    * 共有URLでアクセスすると、該当スポットが自動選択され、その位置にマップがflyToアニメーション（約1.5秒）で滑らかに移動する。これによりユーザーは「スポットの位置に画面が移動した」ことを直感的に理解できる。
    * 詳細画面から「編集する」ボタンで編集モードへ遷移可能。
    * 地図上の長押し（モバイル）または右クリック（デスクトップ）で新規スポット登録フォームを開く。
* **Location Logic**
    * **Default:** 位置情報が許可されている場合は現在地を表示。許可されていない場合は国会議事堂周辺（環境変数で設定可能）。
    * **User Location:** 右下の現在地ボタン（Locateアイコン）をタップすると、Geolocation APIで現在地を取得しズームレベル16で移動。
    * **User Location Marker:** GPSで現在地が取得できた場合、マップ上に青い丸のマーカーで現在地を表示。波紋アニメーションにより視認性を高めている。現在地ボタン押下時に位置が更新される。
    * **Spot URL優先:** スポットURL（`?spot=<id>`）でアクセスした場合、位置情報による自動移動はスキップされ、スポット位置が優先される。
...
* **Input Fields:**
    ...
    2.  **Photo (任意 - 仮運用):** 現場の状況がわかる写真（Cloudflare R2へアップロード）。
        * 撮影ガイダンス表示あり（引きで撮影、演説者・選挙カー・背後の建物が入るように等）。
        * ※ 仮運用として写真なしでの登録を許可中。運用状況を見て必須に戻す可能性あり。
